...
//! flow the release installer performs: download → verify → symlink swap →
...
pub async fn update(releases_url: &str, check_only: bool) -> anyhow::Result<()> {
    let manifest = comet_update::fetch_latest(releases_url).await?;
    ...
    match comet_update::detect_install() {
        InstallKind::Managed { app_root } => {
            ...
            comet_update::stage_headless(releases_url, &manifest, &app_root).await?;
        ...
        InstallKind::MacApp { bundle } => {
            ...
            let staged = comet_update::stage_mac_app(releases_url, &manifest, &data_dir).await?;
