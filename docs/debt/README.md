...
| [D11](D11-remembered-request-mode.md) | A run's remembered `last_request` carries the **previous** turn's runtime mode; three paths prefer it over the chat row, so a mid-chat mode change would not reach a steered or resumed run | 1.1 whole-branch review | ☐ open — owner is 1.8 |
| [D12](D12-permission-axis-version.md) | The permission axis crosses RPC with no version signal: two same-`PROTOCOL_VERSION` builds can pair, and the older one silently runs a request under the default mode | 1.2 Greptile review, premise verified | ☐ open — owner is 1.8 |
| D13 | Codex declares `auto-accept-edits`, but the linked-worktree sandbox workaround silently raises that mode's workspace-write sandbox to `danger-full-access` — trigger is any linked worktree on a slash-named branch, i.e. this repo's own `feature/…` convention — with only a `tracing::warn!` as the signal | 1.3 whole-branch review | ☐ open — 1.3 documents the exception in the capabilities comment; making it visible to the user needs a notice per `user-facing-errors.md`. **1.5 did NOT claim it**, so **1.8 is the remaining candidate owner** |
...
| D24 | **The deny note never reaches Codex, and the field said it would.** Comet persists it, but Codex's decision literals have no message field, so the wire carries `"decline"` and nothing else. Claude's `deny_response(message)` does deliver it | 1.7 live check, scenario 3 | ☐ open — **owner is 1.8**. Fix is to label the field by what the selected provider can honour, not to invent a delivery channel. Symmetry with **D14**'s residual: for Claude the note reaches the model and vanishes from the UI; for Codex it stays in the UI and never reaches the model |
