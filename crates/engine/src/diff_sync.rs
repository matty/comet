...
//! and publishes it locally while reconciling workspace metadata:
//!
...
struct CheckoutEntry {
    ...
    /// asynchronously — watcher setup (budget walk + watch registration) can
...
fn add_entry(inner: &Arc<DiffSyncInner>, identity: CheckoutIdentity, chats: Vec<Chat>) {
    ...
    // MAX_WATCH_DIRS directory entries and watch registration stalls for
    // seconds when the platform's notification service is contended. Doing it
    // inline starved the whole runtime (workspace watches, presence) whenever
    // entries were (re)built, so it runs on the blocking pool and attaches to
    // the entry when ready. Events occurring before attachment are covered by
    // the initial sync; one extra kick after attachment closes the
    // capture-to-attach gap.
