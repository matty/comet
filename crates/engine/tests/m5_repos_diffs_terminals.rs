...
async fn diff_sync_publishes_and_updates_chat_branch() {
    ...
    // Normally the watcher carries this within a debounce; a native watcher is
    // allowed to coalesce or drop events, and a dropped one converges only on
    // the repair tick, so the deadline clears REPAIR_INTERVAL rather than
    // reading loss as failure.
