//! Guard: every provenance citation under `tests/fixtures/` names a corpus
//! path that exists.
//!
...
//! same relative path `comet_harness::capture::corpus_root()` resolves from
...
fn every_fixture_provenance_citation_names_a_corpus_path_that_exists() {
    ...
    for entry in fs::read_dir(&fixtures_dir).expect("read tests/fixtures") {
