...
Scenarios:
  claude: model-discovery, command-discovery, fresh-text, approval, resume, attachment
  codex:  model-discovery, fresh-text, approval, resume, steer, interruption
...
fn capture_config(args: Args) -> Result<CaptureConfig, String> {
    ...
    let discovery = scenario.ends_with("discovery");
...
fn supported_pair(provider: &str, scenario: &str) -> bool {
    matches!(
        ...
        (
            ...
            "codex",
            "model-discovery" | "fresh-text" | "approval" | "resume" | "steer" | "interruption"
        )
    )
}
