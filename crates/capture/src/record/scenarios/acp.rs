...
use crate::record::providers::acp::{AcpProvider, rpc_request};
use crate::record::scenarios::ScenarioInput;
use crate::record::session::Session;
use comet_harness::launch::{LaunchDescriptor, StdioMode};
...
/// separable.
pub(in crate::record) const CODEX_ACP_PACKAGE: &str = "@agentclientprotocol/codex-acp";
pub(in crate::record) const CLAUDE_ACP_PACKAGE: &str = "@agentclientprotocol/claude-agent-acp";
...
/// from `--help` and unguessable, so [`comet_harness::acp::grok::GROK_ARGS`] carries
...
pub(in crate::record) use comet_harness::acp::grok::GROK_ARGS;
...
pub(in crate::record) fn grok_launch(
    input: &ScenarioInput,
    executable: &Path,
) -> anyhow::Result<LaunchDescriptor> {
...
pub(in crate::record) fn codex_acp_launch(
    input: &ScenarioInput,
    executable: &Path,
) -> anyhow::Result<LaunchDescriptor> {
...
pub(in crate::record) fn claude_acp_launch(
    input: &ScenarioInput,
    executable: &Path,
) -> anyhow::Result<LaunchDescriptor> {
...
pub(in crate::record) async fn session_discovery(
    ...
    session
        .send(&rpc_request(
            id,
            "session/new",
            comet_harness::acp::new_session_params(&cwd),
        ))
        .await?;
    session
...
pub(in crate::record) fn resolve_node_executable() -> Option<PathBuf> {
    comet_harness::resolve_cli(
        "NODE_EXECUTABLE",
        "node",
        comet_harness::all_known_dirs(Vec::new()),
    )
}

/// SPAWN for the two Grok run rows (`run-grok`, `steer-grok`).
///
/// Delegates to production's own [`comet_harness::acp::grok::run_launch`] — the same
...
/// production `HermesHarness` and its own `comet_harness::acp::hermes::run_launch`
...
pub(in crate::record) fn run_launch(
    executable: &Path,
    request: &comet_proto::RunRequest,
) -> LaunchDescriptor {
    comet_harness::acp::grok::run_launch(executable, request)
}
...
pub(in crate::record) fn run_request(
    input: &ScenarioInput,
...
pub(in crate::record) async fn run(
    session: &mut Session<AcpProvider>,
    input: &ScenarioInput,
) -> anyhow::Result<()> {
...
async fn prompt_and_wait(
    ...
    session
        .send(&rpc_request(
            prompt_id,
            "session/prompt",
            comet_harness::acp::prompt_params(session_id, text, Vec::new()),
        ))
        .await?;
...
pub(in crate::record) fn steer_request(
    input: &ScenarioInput,
) -> anyhow::Result<comet_proto::RunRequest> {
    let cwd = input.cwd.clone().unwrap_or_else(std::env::temp_dir);
    Ok(comet_proto::RunRequest {
