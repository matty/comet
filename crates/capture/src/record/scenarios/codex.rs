...
use crate::record::providers::codex::{CodexProvider, rpc_request};
...
pub(in crate::record) fn model_discovery_launch(
    ...
    let home = input
        .codex_home
        .clone()
        .or_else(comet_harness::codex::discovery::codex_home)
        .ok_or_else(|| {
            anyhow!("Codex home could not be found. Pass --codex-home and try again.")
        })?;
    let home = crate::record::session::absolute_from_parent(home)?;
    let cwd = input.cwd.clone().unwrap_or_else(std::env::temp_dir);
    Ok(comet_harness::codex::discovery::discovery_launch(
        executable, &home, &cwd,
    ))
}
...
/// `comet_harness::codex::normalize_run_request` here, the one place normalization happens, so
...
fn cheap_codex_request(prompt: &str, input: &ScenarioInput, mode: RuntimeMode) -> RunRequest {
    ...
    comet_harness::codex::normalize_run_request(request)
}
...
async fn start_thread(
    ...
    session
        .send(&rpc_request(
            id,
            "thread/start",
            comet_harness::codex::thread_start_params(request),
        ))
        .await?;
    let reply = session
        .wait_for("JSON-RPC reply", |value| {
...
async fn resume_thread(
    ...
    session
        .send(&rpc_request(
            id,
            "thread/resume",
            comet_harness::codex::thread_resume_params(request, resume_id),
        ))
        .await?;
    let reply = session
        .wait_for("JSON-RPC reply", |value| {
...
async fn start_turn(
    ...
    session
        .send(&rpc_request(
            id,
            "turn/start",
            comet_harness::codex::turn_start_params(request, thread_id, &request.prompt),
        ))
        .await
}
...
/// `resume_thread`) — `comet_harness::codex::run_launch` never reads
...
pub(in crate::record) fn steer_request(input: &ScenarioInput) -> anyhow::Result<RunRequest> {
    Ok(cheap_codex_request(
...
pub(in crate::record) async fn steer(
    session: &mut Session<CodexProvider>,
    input: &ScenarioInput,
) -> anyhow::Result<()> {
    CodexProvider::handshake(session, input).await?;
    ...
    session
        .send(&rpc_request(
            id,
            "turn/steer",
            comet_harness::codex::turn_steer_params(&thread_id, &turn_id, STEER_MESSAGE),
        ))
        .await?;
    session.wait_for_turn_end().await
}
...
/// Same one-call-per-recording contract as `fresh_text_request` above.
pub(in crate::record) fn interruption_request(input: &ScenarioInput) -> anyhow::Result<RunRequest> {
    Ok(cheap_codex_request(
...
pub(in crate::record) async fn interruption(
    session: &mut Session<CodexProvider>,
    input: &ScenarioInput,
) -> anyhow::Result<()> {
    CodexProvider::handshake(session, input).await?;
    ...
    session
        .send(&rpc_request(
            id,
            "turn/interrupt",
            comet_harness::codex::turn_interrupt_params(&thread_id, &turn_id),
        ))
        .await?;
    session.wait_for_turn_end().await
}
...
fn codex_approval_prompt(cwd: &Path) -> String {
    let marker = cwd.join(crate::safety::APPROVAL_MARKER_NAME);
    format!(
...
/// command `crate::safety::approval_marker_command` builds, once, so the wire records a
...
fn approval_on_request_prompt(target: &Path) -> String {
    format!(
        ...
        crate::safety::approval_marker_command(target)
    )
}

/// Same one-call-per-recording contract as `fresh_text_request` above.
pub(in crate::record) fn approval_on_request_request(
    input: &ScenarioInput,
...
fn decision_response(id: Value, decision: &ApprovalDecision) -> String {
    json!({
        "jsonrpc": "2.0",
        "id": id,
        "result": {"decision": comet_harness::codex::approval::decision_literal(decision)},
    })
    .to_string()
}
...
pub(in crate::record) async fn approval(
    ...
    answer_every_approval(session, move || {
        crate::safety::validate_ordinary_approval_cwd(&cwd, expected_cwd_identity.as_ref(), true)
            .map(|_identity| ())
    })
    .await
}
...
pub(in crate::record) async fn approval_on_request(
    session: &mut Session<CodexProvider>,
    input: &ScenarioInput,
) -> anyhow::Result<()> {
    CodexProvider::handshake(session, input).await?;
    ...
    answer_every_approval(session, move || {
        crate::safety::require_empty_approval_target(&target, expected_target_identity.as_ref())
            .map(|_identity| ())
    })
    .await
}
...
mod tests {
    ...
    async fn start_codex_run_session(
        ...
        let launch = comet_harness::codex::run_launch(&executable, &request);
    ...
    fn codex_capture_uses_the_run_command_builder() {
        ...
        let launch = comet_harness::codex::run_launch(&exe, &request);
    ...
    /// `comet_harness::codex::turn_steer_params`/`turn_interrupt_params` computed independently — the
    ...
    async fn capture_steer_and_interrupt_params_match_production_helpers() {
        ...
        assert_eq!(
            steer_line["params"],
            comet_harness::codex::turn_steer_params("th-1", "t-1", STEER_MESSAGE)
        );
        assert_eq!(capture.exit_code, Some(0));
        ...
        assert_eq!(
            ...
            comet_harness::codex::turn_interrupt_params("th-1", "t-1")
        );
        assert_eq!(capture.exit_code, Some(0));
    }
    ...
    /// `ScenarioInput` does not expose — `comet_harness::codex::normalize_run_request` is applied here at
    ...
    async fn recorder_codex_run_preserves_production_linked_worktree_parameters() {
        ...
        let provider_request = comet_harness::codex::normalize_run_request(request.clone());
        ...
        assert_eq!(
            comet_harness::codex::thread_start_params(&provider_request),
            expected_thread
        );
        assert_eq!(
            comet_harness::codex::thread_resume_params(&provider_request, "resume-thread"),
            json!({
                "cwd": worktree.path().display().to_string(),
                "approvalPolicy": "untrusted",
                "sandbox": "danger-full-access",
        ...
        assert_eq!(
            comet_harness::codex::turn_start_params(
                &provider_request,
                "th-1",
                "scenario:capture-fresh"
            ),
            expected_turn
        );
    }
    ...
    /// production `comet_harness::codex::thread_resume_params`, or passes the wrong id.
    ...
    async fn resume_sends_the_production_thread_resume_params() {
        ...
        assert_eq!(
            resume_line["params"],
            comet_harness::codex::thread_resume_params(
    ...
    /// drift between them.
    fn turn_start_and_decisions(capture: &crate::RawCapture) -> (Value, Vec<Value>) {
        let stdin: Vec<Value> = channel_payloads(capture, Channel::Stdin)
            .into_iter()
            .filter_map(|line| serde_json::from_str(line).ok())
            .collect();
    ...
    async fn codex_approval_scenario_answers_every_request_it_sees() {
        ...
        let cwd_identity =
            crate::safety::validate_ordinary_approval_cwd(cwd.path(), None, false).unwrap();
        let input = ScenarioInput {
    ...
    async fn codex_approval_declines_a_grant_when_the_cwd_identity_no_longer_matches() {
        ...
        let mismatched_identity =
            crate::safety::validate_ordinary_approval_cwd(elsewhere.path(), None, false).unwrap();
        let input = ScenarioInput {
            cwd: Some(cwd.path().into()),
            ..ScenarioInput::default()
        };
    ...
    async fn codex_on_request_approval_scenario_answers_every_request_it_sees() {
        ...
        let target_identity =
            crate::safety::require_empty_approval_target(target.path(), None).unwrap();
        let input = ScenarioInput {
    ...
    async fn codex_on_request_approval_declines_a_grant_when_the_target_identity_no_longer_matches()
    {
        ...
        let elsewhere = tempfile::tempdir().unwrap();
        let mismatched_identity =
            crate::safety::require_empty_approval_target(elsewhere.path(), None).unwrap();
        let input = ScenarioInput {
            approval_target: Some(target.path().into()),
            ..ScenarioInput::default()
        };
    ...
    fn every_codex_run_rows_declared_mode_matches_its_request_builder() {
        ...
        for (name, mode) in cases {
            let spec = crate::record::scenarios::scenario("codex", name)
        ...
        let expected: std::collections::BTreeSet<&str> = crate::record::scenarios::SCENARIOS
            .iter()
            .filter(|spec| spec.provider == crate::Provider::Codex && spec.runtime_mode.is_some())
            .map(|spec| spec.name)
            .collect();
        assert_eq!(
            covered, expected,
