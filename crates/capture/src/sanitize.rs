...
/// generated by `crates/capture/src/sheet.rs`) is a snapshot of
/// exactly those names. What it can never name is an **unnamed map key**: a
/// key under one of `surface::MAP_PATHS` with no matching
/// `surface::MapPath::named_children` entry is data, so it is replaced before
/// the path is built, and the entry it produces reads `.modelUsage.{}` with
/// the key counted as a withheld value. `.modelUsage.claude-haiku-4-5-20251001`
/// still appears verbatim, because those two literal lines are on
/// `allowlist/claude.txt` and a listed path is a decision to publish — a
/// *named* child (D123) can instead appear here under its own literal path
/// (`.params.update.rawInput.pattern`, say) with its value still withheld:
/// naming a child makes its key a field name, not its value a published one.
///
    ...
    fn sanitize_value_tree(
        ...
        match value {
            ...
            Value::Object(object) => {
                ...
                // they redact by the same default-deny rule as a value —
                // unless the key is one of that map's own `named_children`
                // (D123): a reviewed field name, exempted from the fold the
                // same way `surface::Visit::walk` exempts it in the sheet.
                // Naming a child here only lets its *key* spelling survive;
                // `key_survives` decides that alone, and the recursion below
                // still runs the child's *value* through the ordinary
                // `allows`/`allows_prefix` check at its real (now literal,
                // not placeholder) path — a named child earns nothing there
                // that an unlisted field wouldn't also default to redacted.
                // The object is rebuilt rather than mutated in place because a
...
mod tests {
    ...
    /// per `crates/capture/src/allowlist/claude.txt`) holds a
    ...
    /// D123: a `named_children` entry (`surface::MapPath`) is a reviewed
    /// *field name*, not a reviewed *value* — the two declarations must not
    /// conflate, or naming a child here would become a backdoor around the
    /// allowlist for exactly the values (a real search string, a real
    /// filesystem path) the map declaration exists to keep out. `pattern`
    /// and `path` survive as keys because `surface::MAP_PATHS` names them
    /// under `.params.update.rawInput`, but neither
    /// `.params.update.rawInput.pattern` nor `.path` is on
    /// `allowlist/acp.txt`, so each value must still redact by the ordinary
    /// default-deny rule.
    #[test]
    fn a_named_map_child_key_survives_but_its_unlisted_value_still_redacts() {
        let out = sanitize_value(
            json!({
                "params": {
                    "update": {
                        "rawInput": {
                            "pattern": "a-real-search-string",
        ...
        let raw_input = out["params"]["update"]["rawInput"]
            .as_object()
            .expect("rawInput stays an object");
        let mut keys: Vec<&str> = raw_input.keys().map(String::as_str).collect();
        keys.sort_unstable();
        assert_eq!(
            keys,
            vec!["path", "pattern"],
            "a named child's key spelling must survive verbatim: {raw_input:?}"
        );
        assert_ne!(
            raw_input["pattern"], "a-real-search-string",
            "a named child's value must still redact by default, got {:?}",
            raw_input["pattern"]
        );
        assert_ne!(
            raw_input["path"], "C:\\Users\\coding\\AppData\\Local\\Temp\\cwd",
            "a named child's value must still redact by default, got {:?}",
            raw_input["path"]
        );
    }
