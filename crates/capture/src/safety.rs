...
use crate::filesystem::has_windows_reparse_point;

// `pub(crate)`, not `pub(super)`: both provider prompt builders that read these
// constants — Claude's `claude_approval_prompt` and Codex's `codex_approval_prompt` — live in
// `crate::record::scenarios::{claude, codex}`, which need to read them from there.
...
pub(crate) const APPROVAL_MARKER_NAME: &str = "capture-marker.txt";
pub(crate) const APPROVAL_MARKER_CONTENT: &str = "capture\n";
...
pub(crate) fn approval_marker_command(target: &Path) -> String {
    #[cfg(windows)]
    {
        let path = target
...
pub(crate) fn repository_root(start: &Path) -> Option<&Path> {
    start
        .ancestors()
...
pub(crate) fn require_empty_approval_target(
    target: &Path,
...
pub(crate) fn validate_on_request_preflight(
    cwd: &Path,
...
pub(crate) fn validate_ordinary_approval_cwd(
    cwd: &Path,
...
#[cfg(windows)]
pub(crate) fn resolve_trusted_powershell(
    cwd: &Path,
    raw_root: &Path,
) -> anyhow::Result<FileIdentity> {
    ...
    if let Some(path) = comet_harness::shell_env::system_path() {
        paths.extend(std::env::split_paths(path));
    }
    ...
    if let Some(home) = comet_harness::home_dir().and_then(|path| std::fs::canonicalize(path).ok())
    {
...
mod tests {
    ...
    use crate::record;
    use crate::test_support::{config, fixture_path, isolated_approval_target, isolated_tempdir};
