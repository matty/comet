...
use crate::types::{CaptureConfig, PartialFailureClass, RawCapture};
...
async fn record_claude(
    ...
    record_provider(
        ...
        input,
        body,
        comet_harness::claude::resolve_claude_executable,
        comet_harness::claude::run_launch,
    )
    .await
}
...
async fn record_codex(
    ...
    record_provider(
        ...
        input,
        body,
        comet_harness::codex::resolve_codex_executable,
        comet_harness::codex::run_launch,
    )
    .await
}
...
async fn record_acp(
    ...
    record_provider(
        ...
        input,
        body,
        crate::record::scenarios::acp::resolve_node_executable,
...
fn codex_fence(
    ...
    if spec.requirements.needs_approval_target {
        ...
        let identity = crate::safety::validate_on_request_preflight(&cwd, target.as_deref())?;
        ...
        return Ok(FenceOutcome {
            ...
            recheck: Some(Box::new(move || {
                let spawn_identity = crate::safety::validate_on_request_preflight(
                    &recheck_cwd,
    ...
    let cwd = launch_cwd()?;
    let trusted = crate::safety::resolve_trusted_powershell(&cwd, &config.raw_root)?;
    let identity = crate::safety::validate_ordinary_approval_cwd(&cwd, None, true)?;
    Ok(FenceOutcome {
...
fn full_access_fence(
    ...
    if crate::safety::repository_root(&canonical).is_some() {
        anyhow::bail!(
...
mod tests {
    ...
    use super::*;
    use crate::test_support::{absolute_program, channel_payloads, config, fixture_path};
    use crate::types::{Channel, CommandSnapshot, Provider};
    ...
    fn claude_model_discovery_capture_uses_the_initialize_builder() {
        ...
        let launch = comet_harness::claude::discovery::model_discovery_launch(&exe, &cwd);
    ...
    fn claude_command_discovery_capture_uses_the_initialize_builder() {
        ...
        let launch = comet_harness::claude::commands::command_discovery_launch(&exe, &cwd);
        let snapshot = CommandSnapshot::from_launch(&launch);
    ...
    fn codex_model_discovery_capture_uses_the_discovery_builder() {
        ...
        let cwd = std::env::temp_dir();
        let launch = comet_harness::codex::discovery::discovery_launch(&exe, &home, &cwd);
        let snapshot = CommandSnapshot::from_launch(&launch);
    ...
    fn every_claude_launch_carries_the_configured_claude_config_dir() {
            ...
            .filter(|spec| spec.provider == Provider::Claude)
        {
            ...
            let (launch, _request) = derive_launch(
                &spec.launch,
                &input,
                &exe,
                comet_harness::claude::run_launch,
    ...
    async fn recorder_persists_structured_host_platform() {
        ...
        assert_eq!(
            ...
            comet_harness::home_dir().map(|path| path.to_string_lossy().into_owned())
        );
        assert_eq!(
    ...
    async fn scenario_launch_and_body_must_share_one_request_builder_call() {
        ...
        fn hazard_body<'a>(
            ...
            Box::pin(async move {
                ...
                let line = comet_harness::claude::wire::user_message_line_with_images(
                    &request.prompt,
                    &[],
                );
    ...
    /// branch existed and protected by policy ("No byte under `crates/capture/tests/corpus/`
    /// changes").
    ///
    ...
    fn every_scenario_launch_matches_its_committed_corpus_manifest() {
        ...
        for spec in SCENARIOS {
            let provider_str = crate::corpus_provider_name(spec.provider, spec.name);
            ...
            let scenario_dirs: Vec<&crate::PromotedScenario> = promoted
                .iter()
                .filter(|scenario| {
